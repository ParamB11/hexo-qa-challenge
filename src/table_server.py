...
PORT = int(os.getenv("TABLE_SERVER_PORT", 8002))
DEFAULT_TOP_K = int(os.getenv("DEFAULT_TOP_K", 5))
DEVICE = os.getenv("SENTENCE_TRANSFORMERS_DEVICE", "cpu")

app = FastAPI(title="Datasheet Table Retrieval Server")
...
def retrieve_tables(request: RetrieveRequest):
    """Retrieve relevant tables from the specified datasheet."""
    if model is None:
        raise HTTPException(status_code=503, detail="Retriever not initialized")
        
    datasheet_dir = os.path.join(DATA_DIR, request.datasheet_id)
    faiss_path = os.path.join(datasheet_dir, "table_index.faiss")
    metadata_path = os.path.join(datasheet_dir, "table_index.metadata.json")

    # 1. Check if index files exist for this datasheet
    if not os.path.exists(faiss_path) or not os.path.exists(metadata_path):
        raise HTTPException(
            status_code=404, 
            detail=f"Missing table index for datasheet_id={request.datasheet_id}. Expected {faiss_path} and {metadata_path}"
        )
    ...
    try:
        ...
        # 5. Format results to match the table_server schema
        results = []
